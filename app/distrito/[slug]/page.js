...
// Función auxiliar para "normalizar" un texto: lo convierte a minúsculas y le quita los acentos.
...
export default function DistritoPage({ params }) {
  ...
  useEffect(() => {
    // Pedimos los 3 archivos de datos a la vez
    Promise.all([
      fetch('/data/resultados_reales.json').then(res => res.json()),
      fetch('/data/resultados_testigo.json').then(res => res.json()),
      fetch('/data/colores.json').then(res => res.json())
    ]).then(([reales, testigo, colores]) => {

      // --- LÓGICA DE NORMALIZACIÓN ROBUSTA ---
      // Creamos nuevos objetos con llaves normalizadas para una búsqueda segura
      const realesNormalizado = Object.fromEntries(
        Object.entries(reales).map(([key, value]) => [normalizeKey(key), value])
      );
      const testigoNormalizado = Object.fromEntries(
        Object.entries(testigo).map(([key, value]) => [normalizeKey(key), value])
      );
      
      setDatos({
        real: realesNormalizado,
        testigo: testigoNormalizado,
        colores: colores
      });
      setIsLoading(false);
    }).catch(err => {
      console.error("Error crítico al cargar archivos JSON:", err);
      setError('No se pudieron cargar los archivos de datos base.');
      setIsLoading(false);
    });
  }, []); // Se ejecuta solo una vez al cargar la página

  if (isLoading) {
    return <main className="p-8 bg-slate-100 min-h-screen text-center"><p>Cargando datos...</p></main>;
  }
  
  if (error) {
     return <main className="p-8 bg-slate-100 min-h-screen text-center"><p className="text-red-500">{error}</p></main>;
  }

  // Ahora que los datos están cargados, renderizamos el dashboard
  return <DistritoDashboard datos={datos} distritoUrl={distrito} />;
}


// Componente hijo que maneja la lógica de la UI una vez que los datos están listos
function DistritoDashboard({ datos, distritoUrl }) {
  const [añoSeleccionado, setAñoSeleccionado] = useState('2023');
  const añosDisponibles = ['2023', '2021', '2019', '2017', '2015', '2013', '2011'];

  const datosProcesados = useMemo(() => {
    const distritoNormalizado = normalizeKey(distritoUrl);
    
    // Búsqueda segura en los datos normalizados
    const datosRealesDistrito = datos.real[distritoNormalizado];
    const datosTestigoDistrito = datos.testigo[distritoNormalizado];

    if (!datosRealesDistrito) return null;

    const datosRealesAño = datosRealesDistrito[añoSeleccionado] || {};
    const datosTestigoAño = (datosTestigoDistrito && datosTestigoDistrito[añoSeleccionado]) ? datosTestigoDistrito[añoSeleccionado] : {};
    
    const calcularPorcentajes = (datosVotos) => {
        if (!datosVotos || Object.keys(datosVotos).length === 0) return {};
    ...
    const prepararDatosGrafico = (porcentajes) => ({
        ...
        datasets: [{
            ...
            backgroundColor: todosLosPartidos.map(p => (datos.colores[p.trim()] || '#cccccc')),
        }]
    });
    ...
    return {
        ...
        existenDatosTestigo: Object.keys(datosTestigoAño).length > 0
    };
  }, [distritoUrl, añoSeleccionado, datos]);

  if (!datosProcesados) {
    return <main className="p-8 bg-slate-100 min-h-screen text-center"><p>No se encontraron datos para el distrito: {distritoUrl}</p></main>;
  }
  ...
  return (
    <main className="p-4 md:p-8 bg-slate-100 font-sans min-h-screen">
      ...
      <header className="text-center mb-10">
        <h1 className="text-5xl font-bold mb-2 text-slate-900">{distritoUrl}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8 max-w-6xl mx-auto">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          {datosProcesados.existenDatosTestigo ?
